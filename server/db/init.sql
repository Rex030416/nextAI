...
-- 这是固定数量的租户分区，不是每个用户一个表或一个数据库。
-- 所有文档检索都必须带 owner_id，PostgreSQL 才能只查询目标分区。
DO $$
DECLARE
...
BEGIN
  FOR partition_index IN 0..31 LOOP
    ...
    EXECUTE format(
      'CREATE TABLE %I PARTITION OF document_chunks
       FOR VALUES WITH (MODULUS 32, REMAINDER %s)',
      partition_name,
      partition_index
    );
    EXECUTE format(
      'CREATE INDEX %I ON %I (document_id)',
      format('%s_document_id_idx', partition_name),
      partition_name
    );
    EXECUTE format(
      'CREATE INDEX %I ON %I USING hnsw (embedding vector_cosine_ops)
       WITH (m = 16, ef_construction = 64)',
      format('%s_embedding_hnsw_idx', partition_name),
      partition_name
    );
    EXECUTE format(
      'CREATE INDEX %I ON %I USING gin (keyword_terms)',
      format('%s_keyword_terms_gin_idx', partition_name),
      partition_name
    );
  END LOOP;
END $$;
